    ...
    screen.renderFunction = (screen, x, y, d) => {
        ...
        try {
            ...
            for (let plat of state.platforms) {
                ...
                drawPlats.push({
                    text: (plat.name || "?") + " -> " + String(mergeDisplayStrings(getDestinationByPlatId(String(new java.lang.Long(plat.id))))),
                    id: String(new java.lang.Long(plat.id))
                });
            }
...
function drawRouteSelectScreen(screen) {
    screen.renderFunction = (screen, x, y, d) => {
        ...
        try {
            ...
            if (state.select)
                if (state.select[0])
                    if (state.cachePlat != state.select[0][0]) {
                        drawRoutes = [];
...
function drawMtrMainSelectionScreen(screen, part, partItem, x, y, d) {
    ...
    g.fillRect(0, 0, w, h);
    g.drawImage(loadResource("img", "mtrsteamloco:cfg/sr2.png"), 0, 0, w, h, null);
    ...
    for (let i = 0; i < part; i++) {
        ...
        if (state.rollInfo)
            if (mouseInBox(mx, my, currentX + w * 0.01, h * 0.15, partWidth - w * 0.02, h * 0.65)) {
        ...
        if (partItem[i].length == 0) {
            ...
            g.drawString(
                ComponentUtil.getString(ComponentUtil.translatable("cfg.no_available_item")),
                currentX +
                    partWidth * 0.5 -
                    g.getFontMetrics(font.deriveFont(Font.PLAIN, lineHeight * 0.35)).stringWidth(ComponentUtil.getString(ComponentUtil.translatable("cfg.no_available_item"))) * 0.5,
                h * 0.5
            );
        }
        for (let j = 0; j < partItem[i].length; j++) {
            ...
            let selected = select[i] !== undefined ? currentSelecIndex > -1 : false;
    ...
    g.setFont(font.deriveFont(Font.PLAIN, lineHeight * 0.35));
    g.drawString(
        ComponentUtil.getString(ComponentUtil.translatable("cfg.confirm")),
        w * 0.5 - g.getFontMetrics(font.deriveFont(Font.PLAIN, lineHeight * 0.35)).stringWidth(ComponentUtil.getString(ComponentUtil.translatable("cfg.confirm"))) * 0.5,
        h * 0.91
    );
