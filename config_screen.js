    ...
    screen.renderFunction = (screen, x, y, d) => {
        ...
        try {
            ...
            for (let i = 1; i <= configPageList.length; i++) {
                ...
                if (item.type != "str")
                    g.fillRoundRect(w * 0.775, drawY + lineHeight * 0.05, w * 0.2, lineHeight * 0.9, lineHeight, lineHeight);
                else
                    g.fillRoundRect(w * 0.5, drawY + lineHeight * 0.05, w * 0.475, lineHeight * 0.9, lineHeight, lineHeight);
                ...
                if (item.type != "str")
                    g.drawString(buttonText, w * 0.875 - 0.5 * g.getFontMetrics(font.deriveFont(lineHeight * 0.6)).stringWidth(buttonText), drawY + lineHeight * 0.5 + g.getFontMetrics(font.deriveFont(lineHeight * 0.6)).getHeight() * 0.5);
                else
                    g.drawString(buttonText, w * 0.7375 - 0.5 * g.getFontMetrics(font.deriveFont(lineHeight * 0.6)).stringWidth(buttonText), drawY + lineHeight * 0.5 + g.getFontMetrics(font.deriveFont(lineHeight * 0.6)).getHeight() * 0.5);

                if (state.clickInfo) if (state.clickInfo.x >= pdbox.x1 && state.clickInfo.x <= pdbox.x2 && state.clickInfo.y >= pdbox.y1 && state.clickInfo.y <= pdbox.y2) {
                    if (item.type == "num") {
                        if (state.inEditing != -1) if (state.inEditing != i) {
                ...
                if (state.inEditing == i) {
                    ...
                    if (item.type == "num") {// 预处理：整数模式强制清理现有小数点
                        ...
                        if (state.cacheChr)
                            for (let char of state.cacheChr) {
                                if (/\d/.test(char)) {  // 数字直接追加
                                    current += char;
                                } else if (char === '.' && !targetVal.isInt) {  // 小数模式处理
                                    if (!current.includes('.')) {  // 仅允许第一个小数点
                                        current += char;
                                    }
                                }
                            }
        ...
        try {
            ...
            if (state.selected != -1) {
                ...
                if (state.content.cache != state.selected) {
                    ...
                    if (item.val.contentImage) state.contentHeight = w * 0.55 / item.val.contentImage.width * item.val.contentImage.height + lineHeight * 0.25 + g.getFontMetrics(contentFont).getHeight() * (state.content.text.length + 1);
                ...
                if (item.val.contentImage) {
                    ...
                    g.drawImage(loadRes(resSc, "image", item.val.contentImage.img), w * 0.375, lineHeight * 0.8 + state.contentScoll, w * 0.55, w * 0.55 / item.val.contentImage.width * item.val.contentImage.height, null);
                ...
                if (state.clickInfo) if (state.clickInfo.x >= w * 0.35 && state.clickInfo.x <= w * 0.95 && state.clickInfo.y >= lineHeight * 6.8 && state.clickInfo.y <= lineHeight * 7.5 && y * posRateY < screen.state.lineHeight * 6.25) {
                    state.eyecandy.block.putCustomConfig(String(state.saveKey), String(item.val.content));
                    state.eyecandy.block.sendUpdateC2S();
                    state.eyecandy.ctx.setDebugInfo(state.saveKey, item.val.content);
                    MinecraftClient.displayMessage(state.saveKey, false);
